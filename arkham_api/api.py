...
def load_fact() -> pd.DataFrame:
    ...
    blob   = bucket.blob(f"{GCS_PREFIX}/model/medicion_dia.parquet")

    if not blob.exists():
        raise HTTPException(
            status_code=503,
            detail="Los datos no están disponibles. Llama a POST /refresh primero."
        )

    data = blob.download_as_bytes()
    df   = pd.read_parquet(io.BytesIO(data))
    df["date_id"] = df["date_id"].astype(str)
    logger.info(f"Datos cargados desde google: {len(df):,} registros")
    return df
...
def get_data(
    ...
    min_percent_outage: Optional[float] = Query(
        ...
        ge=0,
        le=100,
        description="Filtrar registros con % de outage >= este valor. Ej: 0.0",
    ),
    ...
    sort_by: str = Query(default="date_id", description="Campo por el que ordenar (date_id,outage_mw (días con menos outage primero),percent_outage(días con más outage primero))"),
...
def refresh():
    ...
    try:
        logger.info("Verificando última fecha en storage")
        
        # Detectar la fecha más reciente ya almacenada
        start_arg = None
        try:
            client     = gcs.Client()
            bucket     = client.bucket(GCS_BUCKET)
            ...
            if blob.exists():
                data       = blob.download_as_bytes()
                df_exist   = pd.read_parquet(io.BytesIO(data))
                last_date  = pd.to_datetime(df_exist["date_id"]).max()
                # Empezar desde el día siguiente al último registro
                start_arg  = (last_date + pd.Timedelta(days=1)).strftime("%Y-%m-%d")
                logger.info(f"Último registro en GCS: {last_date.date()} — extrayendo desde {start_arg}")
            else:
                logger.info("No hay datos previos en GCS — extracción completa.")
        except Exception as e:
            logger.warning(f"No se pudo determinar fecha incremental: {e} — extracción completa.")

        # Construir comando con o sin --start
        cmd = [sys.executable, str(connector_path)]
        if start_arg:
            cmd += ["--start", start_arg]
        ...
        results["incremental"] = start_arg or "completa"
